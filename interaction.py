...
class Model(object):
    ...
    def __init__(self, num_bodies=3, body_rad=10):
        ...
        while len(body_centers) < num_bodies:
            x = random.randint(0+body_rad, screen_width-body_rad)
            y = random.randint(0+body_rad, screen_height-body_rad)
            new_center = Point(x, y)
            if not self.too_close(new_center, body_centers, 2*body_rad):
                body_centers.append(new_center)
...
class PyGameKeyboardController(object):
    ...
    def handle_event(self, event, model):
        ...
        #   Now audio_unit handles everything
            ...
            audio_unit.play_sample_num(12)
            self.speed_random(model)
        elif event.key == pygame.K_u:
            audio_unit.play_sample_num(13)
            self.speed_random(model)
        elif event.key == pygame.K_i:
            audio_unit.play_sample_num(9)
            self.speed_random(model)
        elif event.key == pygame.K_o:
            audio_unit.play_sample_num(10)
            self.speed_random(model)
        elif event.key == pygame.K_p:
            audio_unit.play_sample_num(11)
            self.speed_random(model)
...
if __name__ == '__main__':
    ...
    pygame.mixer.music.load("creature_feature.wav") 
